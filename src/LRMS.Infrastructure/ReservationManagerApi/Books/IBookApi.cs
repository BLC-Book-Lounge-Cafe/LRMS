...
public interface IBookApi
{
    ...
    [Get("/api/v1/books")]
    Task<HttpResponseMessage> GetBookReservations([Refit.Query] int? book_id, [Refit.Query] string? reserved_at,
        [Refit.Query] int? page_number, [Refit.Query] int? page_size, CancellationToken ct = default);

    [Delete("/api/v1/books/{id}")]
    Task<HttpResponseMessage> DeleteBookReservation(int id, CancellationToken ct = default);
}
